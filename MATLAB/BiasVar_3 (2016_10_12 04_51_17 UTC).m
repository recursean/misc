function [bias, var] = BiasVar_3(Xs, ys, XTest, yTest)
    ...
    for k=1:size(XTest,1)
        ...
        %z2 = [TransXTest1(1); TransXTest1(1)^2;TransXTest1(2); TransXTest1(2)^2;TransXTest1(3); TransXTest1(3)^2]; 
    ...
    for m=1:size(g2,2)
        ...
        for n=1:size(XTest,1)
             TransXTest1 = Phi1(XTest(n));
             TransXTest2 = Phi2(XTest(n));
             
             gX2 = (currentG(2) * TransXTest1(2) + currentG(3) * TransXTest1(3) + currentG(1));
             gBarX2 = (abar2 * TransXTest1(2) + bbar2 * TransXTest1(3) + cbar2);  
             sumOverX = sumOverX + (gX2 - gBarX2)^2;
        end   
        sumOverG = sumOverX / size(XTest,1);
    end
    var(2) = sumOverG / size(Xs,2);
